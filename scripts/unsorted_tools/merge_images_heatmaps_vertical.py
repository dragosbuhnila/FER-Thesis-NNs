...
MODEL_NAME_W = 180  # left column width for model name
CELL_W = 360
CELL_H = 240
TOP_MARGIN_H = 28
H_SPACING = 12
...
def create_vertical_text_image(text, font):
    """
    Create an RGBA image with the text rendered horizontally, then rotate it to vertical.
    Returns the rotated image (RGBA) so it can be pasted with transparency.
    """
    # measure text size
    temp_img = Image.new("RGBA", (1,1), (0,0,0,0))
    temp_draw = ImageDraw.Draw(temp_img)
    tw, th = get_text_size(temp_draw, text, font)
    # create image with some padding
    pad = 4
    txt_img = Image.new("RGBA", (tw + pad*2, th + pad*2), (0,0,0,0))
    d = ImageDraw.Draw(txt_img)
    d.text((pad, pad), text, font=font, fill=(0,0,0,255))
    # rotate -90 so text reads top->bottom (adjust if you prefer opposite direction)
...
def compose_vertical_page(models_slice, page_index, output_dir):
    ...
    for row_idx, model_name in enumerate(models_slice):
        ...
        # --- draw model name vertically centered in the left column ---
        # create rotated text image
        vert_img = create_vertical_text_image(model_name, name_font)
        rw, rh = vert_img.size
