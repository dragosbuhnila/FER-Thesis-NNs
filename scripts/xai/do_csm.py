...
def get_test_set_name_from_run_name(run_name):
    ...
    """
    if "occluded" in run_name:
        return "occluded"
    elif "original" in run_name:
        return "original"
    elif "180-rotated" in run_name:
        return "180-rotated"
    else:
        raise ValueError(f"Unable to determine test set name from run name: {run_name}")
...
elif test_set_name == "180-rotated":
    TEST_SET_IMAGES_PATH = ADELE_180ROTATED_TEST_SET_IMAGES_PATH
    EXTRACT_IMAGE_INDEX_FUNCTION = extract_image_index_original_or_180rotated

# Redirect output if specified
if args.redirect_output:
    LOG_FILE_PATH = os.path.join(CONSOLE_OUTPUTS_PATH, f"{get_timestamp()}_do-generate-bubbles-planes_from_{args.run_name}.log")
